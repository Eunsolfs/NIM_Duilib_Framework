	...
	bool fullxtiled /*= true*/, bool fullytiled /*= true*/, int nTiledMargin /*=0*/)
{
	...
	// Â¶ÇÊûúÊ∫ê‰ΩçÂõæÂ∑≤ÁªèÊåâÁÖßDPIÁº©ÊîæËøáÔºåÈÇ£‰πàÂØπÂ∫îÁöÑcorner‰πüÁº©Êîæ‰∏Ä‰∏ã
	if (bBitmapDpiScale)
...
void RenderContext_GdiPlus::DrawRoundRect(const UiRect& rc, const CSize& roundSize, int nSize, DWORD dwPenColor)
{
	...
	// Ë£ÅÂâ™Âå∫Âüü‰∏çËÉΩ‰ΩúÁîªÔºåÂØºËá¥ËæπÊ°ÜÊúâÊó∂‰∏çÂÖ®ÔºåÂæÄÈáåÊî∂Áº©‰∏Ä‰∏™ÂÉèÁ¥†
	// UiRect rcInflate = rc;
...
void RenderContext_GdiPlus::DrawText(const UiRect& rc, const std::wstring& strText, DWORD dwTextColor, const std::wstring& strFontId, UINT uStyle, BYTE uFade /*= 255*/, bool bLineLimit /*= false*/, bool bFillPath /*= false*/)
{
	...
	if ((uStyle & DT_TOP) != 0) {
		...
		if (fontInfo->sFontName == L"Êñ∞ÂÆã‰Ωì") {
...
#define USE_COLOR_MATRIX 0
	...
	//A 5√ó5 color matrix is a homogeneous matrix for a 4 - space transformation.
	//The element in the fifth row and fifth column of a 5√ó5 homogeneous matrix must be 1, 
...
ui::UiRect RenderContext_GdiPlus::MeasureText(const std::wstring& strText, const std::wstring& strFontId, UINT uStyle, int width /*= DUI_NOSET_VALUE*/) {	
	...
	if (width == DUI_NOSET_VALUE) {
		...
		if ((uStyle & DT_SINGLELINE) != 0) {
			...
			graphics.MeasureString(L"ÊµãËØï", 2, &font, rcEmpty, &stringFormat, &bounds);
...
ui::UiRect RenderContext_GdiPlus::MeasureTextEx(const std::wstring& strText, const std::wstring& strFontId, UINT uStyle, int width) {
	...
	// ≤Œøº£∫GDI+—ßœ∞º∞¥˙¬Î◊‹Ω·÷Æ------Œƒ±æ”Î◊÷ÃÂ https://blog.csdn.net/harvic880925/article/details/9097319
	Gdiplus::StringFormat stringFormat = Gdiplus::StringFormat::GenericTypographic();
	int formatFlags = 0;
	...
	if (width == DUI_NOSET_VALUE) {
		...
		Gdiplus::RectF layoutRect(0, 0, width, 0);//∏ﬂ∂»Œ™0£¨»√∆‰◊‘∂Øº∆À„∏ﬂ∂»
