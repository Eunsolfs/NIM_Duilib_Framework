...
BOOL CTxtWinHost::Init(RichEdit *re, const CREATESTRUCT *pcs)
{
	...
	//Ëß£ÂÜ≥32‰ΩçÁ≥ªÁªü‰∏ãË∑®Á™óÂè£Èó¥ÊãñÊãΩÊñáÂ≠óÂú®win7Âèäwin7‰ª•‰∏ãÁ≥ªÁªü‰∏ä‰ºöÂá∫Áé∞ÈáçÂ§çÁöÑÈóÆÈ¢òÔºà64‰ΩçÊöÇ‰∏ç‰øÆÂ§çÔºâ lty 20170714
...
{
	ASSERT(FALSE); //ÊöÇÊó∂Ê≥®ÈáäÊéâÔºå‰∏çÁü•ÈÅìËøô‰ª£Á†ÅÊúâÂï•Áî®   by panqinke 2014.5.6
...
void RichEdit::SetText(const std::wstring& strText)
{
	...
	//–ﬁ∏¥¡ÀRichEditÕ¨ ±…Ë÷√Width∫ÕHeightŒ™Auto ±Œﬁ∑®º∆À„∏ﬂ∂»µƒŒ Ã‚£®¥À ±±ÿ–Î…Ë÷√MaxWidth£©
...
void RichEdit::SetImmStatus(BOOL bOpen)
{
	...
	if (hwnd != NULL)
	{
		// Â§±ÂéªÁÑ¶ÁÇπÊó∂ÂÖ≥Èó≠ËæìÂÖ•Ê≥ï
		HIMC hImc = ::ImmGetContext(hwnd);
    // Â§±ÂéªÁÑ¶ÁÇπÊòØ‰ºöÊääÂÖ≥ËÅîÁöÑËæìÂÖ•Ê≥ïÂéªÊéâÔºåÂØºËá¥Êó†Ê≥ïÊó†Ê≥ïËæìÂÖ•‰∏≠Êñá
...
// Â§öË°åÈùûrichÊ†ºÂºèÁöÑricheditÊúâ‰∏Ä‰∏™ÊªöÂä®Êù°bugÔºåÂú®ÊúÄÂêé‰∏ÄË°åÊòØÁ©∫Ë°åÊó∂ÔºåLineDownÂíåSetScrollPosÊó†Ê≥ïÊªöÂä®Âà∞ÊúÄÂêé
// ÂºïÂÖ•iPosÂ∞±ÊòØ‰∏∫‰∫Ü‰øÆÊ≠£Ëøô‰∏™bug
...
CSize RichEdit::EstimateSize(CSize szAvailable)
{
	...
	if (size.cx == DUI_LENGTH_AUTO || size.cy == DUI_LENGTH_AUTO) {
		...
		//https://github.com/xmcy0011/NIM_Duilib_Framework/commit/abea331f570c903228d333cda83358dcf7cec887
		if (size.cx == DUI_LENGTH_AUTO && size.cy == DUI_LENGTH_AUTO) {
			// fixed_by xmcy0011@sina.com 2021-04-22 …Ë÷√width=auto && height=auto ±£¨RichEditŒﬁ∑®◊‘∂Øº∆À„øÌ∏ﬂµƒŒ Ã‚
			// ¥À ±“™…Ë÷√◊Ó¥ÛøÌ∂»£¨∑Ò‘ÚŒﬁ∑®º∆À„°£
...
void RichEdit::SetPos(UiRect rc)
{
	...
	for( auto it = m_items.begin(); it != m_items.end(); it++ ) {
        ...
        if( pControl->IsFloat() ) {
            ...
            pControl->SetPos(rc); // ÊâÄÊúâÈùûfloatÂ≠êÊéß‰ª∂ÊîæÂ§ßÂà∞Êï¥‰∏™ÂÆ¢Êà∑Âå∫
        }
    }
}
...
void RichEdit::OnKeyDown(EventArgs& event)
{
	if (event.wParam == VK_RETURN && ::GetAsyncKeyState(VK_SHIFT) >= 0)	{
		...
		if (!strClipText.empty()) {
			...
			SetClipBoardText(strNum); //‰øÆÊîπÂâ™ÂàáÊùøÂÜÖÂÆπ‰∏∫Á∫ØÊï∞Â≠ó
			nbase::ThreadManager::PostTask([strClipText]() { SetClipBoardText(strClipText); }); //Á≤òË¥¥ÂÆåÂêéÂèàÊääÂâ™ÂàáÊùøÂÜÖÂÆπÊîπÂõûÊù•
		}
	}
...
//Ê†πÊçÆpointÊù•hittestËá™ÂÆö‰πâlinkÁöÑÊï∞ÊçÆÔºåËøîÂõûtrueË°®Á§∫Âú®link‰∏äÔºåinfoÊòØlinkÁöÑËá™ÂÆö‰πâÂ±ûÊÄß
...
//----------------‰∏ãÈù¢ÂáΩÊï∞Áî®‰ΩúËæÖÂä© Â≠óËäÇÊï∞ÈôêÂà∂
...
void SetClipBoardText(const std::wstring &str)
{
	...
	HGLOBAL hMem = ::GlobalAlloc(GMEM_MOVEABLE, (len + 1) * sizeof(wchar_t)); //ÂàÜÈÖçÂÖ®Â±ÄÂÜÖÂ≠ò  
	if (!hMem) {
		::CloseClipboard();
		return;
	}

	wchar_t* lpStr = (wchar_t*)::GlobalLock(hMem); //ÈîÅ‰ΩèÂÜÖÂ≠òÂå∫ 
	::memcpy(lpStr, str.c_str(), len * sizeof(wchar_t)); //ÊääÊï∞ÊçÆÊã∑Ë¥ùËÄÉÂÖ®Â±ÄÂÜÖÂ≠ò‰∏≠
	lpStr[len] = wchar_t(0); //Â≠óÁ¨¶‰∏≤Êú´Â∞æËÆæ‰∏∫'\0'
	::GlobalUnlock(hMem); //ÈáäÊîæÈîÅ 

	::SetClipboardData(CF_UNICODETEXT, hMem); //ÊääÂÜÖÂ≠ò‰∏≠ÁöÑÊï∞ÊçÆÊîæÂà∞Ââ™ÂàáÊùø‰∏ä
	::CloseClipboard(); //ÂÖ≥Èó≠Ââ™ÂàáÊùø	
}
